...
int main (int argc, char *argv[])
{
    ...
    try {
        ...
        FloatVector *v = segment.find<FloatVector> (handle_name_read).first;
        std::cout<<"C++ from Python = [";
        for(int i=0;i<v->size(); i++){
            std::cout<<(*v)[i]<<", ";
        }
        std::cout<<"]";
        segment.destroy<FloatVector>(handle_name_read);

        const ShmemAllocator alloc_inst (segment.get_segment_manager());
        FloatVector *myvector = segment.construct<FloatVector>(handle_name_write)(alloc_inst);
        for(int i=0; i<10; i++){
