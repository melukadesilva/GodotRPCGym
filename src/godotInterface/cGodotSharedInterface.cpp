...
cGetSharedMemoryTensor::cGetSharedMemoryTensor(const std::string &name) {
	segment_name = new std::string(name);
	try {
		...
		std::cout << "Segment error " << boost::diagnostic_information(e) << std::endl; 
	} catch (const char *s) {
		std::cout << "cSharedMemoryTensor " << s << std::endl;
	}
}

cGetSharedMemoryTensor::~cGetSharedMemoryTensor() {
	delete segment;
	delete segment_name;
}
...
cPersistentFloatTensor* cGetSharedMemoryTensor::findFloatTensor(const std::string &name){
	...
	*/
	//Ref<cPersistentFloatTensor> tensor(memnew(cPersistentFloatTensor(shared_vector)));
	cPersistentFloatTensor *tensor = new cPersistentFloatTensor(shared_vector, name, segment);
	return tensor;
}
...
cSharedMemorySemaphore::cSharedMemorySemaphore(const std::string &sem_name) {
	name = new std::string(sem_name);
	//mapped_region *region = get_region();
	try{
		shared_memory_object object(open_only, name->c_str(), read_write);
		region = new mapped_region(object, read_write);
		if (region == NULL) {
			std::cout << "Cannot find the semaphore " << name->c_str() << std::endl;
		}
		else {
			std::cout << "Found semaphore " << name->c_str() << std::endl;
		}
...
/*
// there are 2 constructors in the base class; select the dummy one and use the dervided class constructor to find the semaphore
cGetSharedMemorySemaphore::cGetSharedMemorySemaphore(const std::string &sem_name) : cSharedMemorySemaphore::cSharedMemorySemaphore() {
	name = new std::string(sem_name);
	//mapped_region *region = get_region();
	try{
		shared_memory_object object(open_only, name->c_str(), read_write);
